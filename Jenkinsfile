...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="superbart/cicd_guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'master', url:'https://github.com/superbart/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'superbart.j@gmail.com')
